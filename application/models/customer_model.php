...
class Customer_model extends crm_model {
    ...
    public function sales_agent_list() {
        ...
        $this->db->select('userid, first_name, last_name, sales_code');
        $this->db->where('level', 4);
        $this->db->where_in('sales_code',  array_keys($config['sales_codes']));
        $res = $this->db->get($this->cfg['dbpref'].'users')->result_array();
		if(!empty($res))
		return $res;	
		else
		return array(); 
