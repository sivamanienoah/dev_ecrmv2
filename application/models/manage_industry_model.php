 ...
 *
 * @class 		Manage_industry_model
 * @extends		crm_model (application/core)
 * @classes     Model
 * @author 		eNoah
 */

class Manage_industry_model extends crm_model {
    
	/*
	*@construct
	*@Manage Service Model
	*/
    function Manage_industry_model() {
       parent::__construct();
	   $this->load->helper('custom_helper');
    }

	/*
	*@Get industry for Search
	*@Manage_industry_model
	*/
	public function get_industry($search = FALSE) {
		$this->db->select('*');
		$this->db->from($this->cfg['dbpref'].'industry');
		if ($search != false) {
