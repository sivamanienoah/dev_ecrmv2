...
class Report_lead_owner_model extends crm_model {
    
    function Report_lead_owner_model() {
        parent::__construct();
		$this->load->helper('lead_stage_helper');
		$this->stg = getLeadStage();
    }
    
    public function getLeadReportByOwner($options = array())
    {
    	...
    	if(!empty($options['worth']) && $options['worth'] != 'null')
		{
			...
			$where.= ')';
			$this->db->where($where);
			//$this->db->where_in('jb.expect_worth_amount',$worth);
		}   	
