...
class Report_lead_industry_model extends crm_model {
    
    function Report_lead_ser_model() {
        parent::__construct();
		$this->load->helper('lead_stage_helper');
		$this->stg = getLeadStage();
    }
    
    public function getLeadReportByIndustry($options = array())
    {
		...
		if(!empty($options['industry']) && $options['industry'] != 'null'){
			$industry = explode(',',$options['industry']);
			$this->db->where_in('jb.industry', $industry);
		}
    	...
    	$this->db->join($this->cfg['dbpref'].'lead_services jc','jc.sid = jb.lead_service','INNER');
    	$this->db->join($this->cfg['dbpref'].'industry ji','ji.id = jb.industry','INNER');
    	$this->db->join($this->cfg['dbpref'].'users mu','mu.userid = jb.modified_by','LEFT');
    	$this->db->where_in('jb.lead_stage', $this->stg);
    	$this->db->order_by('jb.industry','ASC');
	...
	{		
		$query = $this->db->get_where($this->cfg['dbpref'].'industry', array('status'=>1));
