...
class Report_active_lead_model extends crm_model {
    ...
    public function getActiveLead($options = array())
    {
    	...
    	/*if(!empty($options['range']) && $options['range'])
    	{
    		$isSelect=$options['range'];	
    	}*/
    	...
    	if(!empty($options['start_date']))
		{
			...
			$this->db->where('date(jb.date_modified) >=',$start_date);
		...
		if(!empty($options['end_date']))
		{
			...
			$this->db->where('date(jb.date_modified) <=',$end_date);
		}
