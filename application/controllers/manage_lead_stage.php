...
class Manage_lead_stage extends crm_controller {
	...
	function leadStg_add($update = false, $id = false) {
		...
		$data['cb_status'] = $this->manage_lead_stage_model->get_num_row('jobs', array('job_status' => $id));
	...
	function ajax_edit_leadstg() {
        ...
        if (is_array($errors)) {
			...
			$data['cb_status'] = $this->manage_lead_stage_model->get_num_row('jobs', array('job_status' => $this->input->post('lead_stage_id')));
    ...
    function ajax_delete_leadStg() {	
		...
		if ($this->session->userdata('delete') == 1) {
			...
			if (is_array($errors)) {
				...
				$stat = $this->manage_lead_stage_model->get_num_row('jobs', array('job_status' => $stage_id));
				if ($stat==0) {		
	...
	function ajax_check_status_lead_stage() {
		$rows = $this->manage_lead_stage_model->get_num_row('jobs', array('job_status' => $this->input->post('data')));
		$res = array();
		if($rows == 0) {
			$res['html'] .= "YES";
		} else {
