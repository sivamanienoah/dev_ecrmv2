	...
	*/
	...
	{	//echo'test';exit;
		...
		if(count($skill_ids)>0 && !empty($skill_ids)) {
			...
			$members_by_skills_query = "SELECT empname as emp_name, username FROM crm_timesheet_data  WHERE practice_id != 0 AND (start_time >='".date('Y-m-d', strtotime($start_date))."' ) AND (start_time <='".date('Y-m-t', strtotime($end_date))."' ) AND dept_id IN ('".$depatids."') AND practice_id IN ('".$pracids."') AND skill_id IN ('".$skill_ids."') GROUP BY empname ";
