...
class Dashboard extends crm_controller {
    ...
    function utilization_metrics_beta() {
        ...
        if (count($skill_ids) > 0 && !empty($skill_ids) && count($department_ids) > 0 && !empty($department_ids)) {
            ...
            $qry1 = $timesheet_db->query("SELECT v.username,v.emp_id,concat(v.first_name,' ',v.last_name) as emp_name FROM `v_emp_details` v join enoah_times t on v.username=t.uid where v.department_id in ($dids) and v.skill_id in ($sids) and t.start_time between '$start_date' and '$end_date' group by v.username order by v.username asc");
    ...
    function get_project_by_practice() {
        ...
        if ($this->input->post("prac_id")) {
            ...
            if (count($p_ids) > 0 && !empty($p_ids)) {
                // $data['department_ids'] = $p_ids;
                // $data['filter_area_status'] = 1;
              ...
              $this->db->where($depatwhere);
              } */

            $res_query = $this->db->get();

            // echo "<pre>"; print_R($p_ids);
    ...
    public function cost_report() { //echo'test';exit;
        // echo'<pre>';print_r($this->input->post());exit;
        if (in_array($this->userdata['role_id'], array('8', '9', '11', '13', '14'))) {
            redirect('project');
        }
        ...
        $member_ids = $this->input->post("member_ids");
        $entity_ids = $this->input->post("entity_ids");
        $project_reslt = $this->input->post("project_res");
    ...
    public function cost_report_export() {
        ...
        if (!empty($project_reslt) && count($project_reslt) > 0 && ($project_reslt != 'null')) {
        ...
        if (count($skill_ids) > 0 && !empty($skill_ids) && ($skill_ids != 'null')) {
            $skill_ids_arr = array();
            $skill_ids_arr = @explode(",", $skill_ids);
            if (!empty($skill_ids_arr) && count($skill_ids_arr) > 0) {
                $this->db->where_in('t.skill_id', $skill_ids_arr);
            }
        }
