...
class It_service_dashboard extends crm_controller 
{
	...
	function index()
	{
		...
		if($this->input->post("filter")=="") {
			...
			foreach($practice_array as $prarr){
				...
				echo 'tot_cm_irvals<br>'.$tot_cm_irvals += $projects['cm_irval'][$prarr];
				echo 'tot_cm_dc_tot<br>'.$tot_cm_dc_tot += $temp_cm_utd_cost;
				
				echo 'tot_dc_vals<br>'.$tot_dc_vals += $projects['irval'][$prarr];
				echo 'tot_dc_tots<br>'.$tot_dc_tots += $temp_ytd_utilization_cost;
	...
	*/
	public function get_direct_cost_val($practice, $month=false, $start_date=false, $end_date=false)
	{
		...
		// $this->db->where('l.lead_id', 59); // for temporary - load some data only
		// $this->db->where('t.username', 'suresha.t'); // for temporary - load some data only
