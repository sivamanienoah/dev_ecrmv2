...
class User_email_template extends CRM_Controller {
	...
	*/
	...
	public function __construct() 
	{
		...
		$this->load->model('user_email_template_model');
	...
	*/
    ...
    // ini_set('display_errors', 1);
		...
		$data['email_template'] = $this->user_email_template_model->get_email_templates();
		//print_r($data['email_template']); exit;
		$this->load->view('user_email_template/email_template_view', $data); 
    }
	...
	public function add_email_template($update = false, $id = false) {
	   ...
	   $update_item = $this->input->post('update_item');	
		...
		if ($update == 'update' && preg_match('/^[0-9]+$/', $id)) {	
            $src = $this->user_email_template_model->get_row('custom_email_template', array('temp_id' => $id));
		...
		if ($this->validation->run() != false) {
			...
			if ($this->user_email_template_model->update_row('custom_email_template', array('temp_id' => $id), $updt_item)) {
                    $this->session->set_flashdata('confirm', array('Email Template Updated!'));
                    redirect('user_email_template');
                }
            } else {
	            ...
	            $this->user_email_template_model->insert_row('custom_email_template', $ins_item);
                $this->session->set_flashdata('confirm', array('Email Template Added!'));
                redirect('user_email_template');
            }
		}
		$data['def_templates']=$this->user_email_template_model->get_default_templates();
		$this->load->view('user_email_template/email_template_add', $data);
	}
	...
	*/
	...
	public function get_template()
	{
		...
		$temp_content =$this->user_email_template_model->get_template_content($temp_id);
