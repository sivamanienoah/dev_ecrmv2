<?php
class report_lead_assignee extends crm_controller {
...
public $userdata;
    ...
    public function get_lead_report() {
    	...
    	if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) AND strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {    		
   			$this->load->view('report/lead_report_assignee_view',$data);
		}else{
    		return $this->load->view('report/lead_report_assignee_view',$data,true);
		}    	
    }
    ...
    public function excelExport() {
    	...
    	if($res['num']>0)
    	{
			...
			$this->excel->getActiveSheet()->setCellValue('A1', 'Lead assignee');
			...
			$this->excel->getActiveSheet()->setCellValue('F1', 'Lead Owner');
    		...
    		foreach($leads as $lead) {		
    			...
    			$this->excel->getActiveSheet()->setCellValue('F'.$i, $lead->owner_first_name.' '.$lead->owner_last_name);
    			...
    			$cnt++;
    			if(empty($leads[$cnt]->assigned_id) || $leads[$cnt]->assigned_id != $lead->assigned_id)
    			{
    				$end = $i;
    				$this->excel->getActiveSheet()->setCellValue('A'.$st, $lead->assigned_first_name.' '.$lead->assigned_last_name);
			...
			$filename='Lead_report_by_assignee.xls'   ; //save our workbook as this file name
