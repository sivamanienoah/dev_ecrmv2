<?php
class report_lead_owner extends crm_controller {
...
public $userdata;
    ...
    public function get_lead_report() {
    	...
    	$res = $this->report_lead_owner_model->getLeadReportByOwner($options);   	
    	...
    	if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) AND strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {    		
   			$this->load->view('report/lead_report_owner_view',$data);
		}else{
    		return $this->load->view('report/lead_report_owner_view',$data,true);
		}    	
    }
    ...
    public function excelExport() {
    	...
    	if($res['num']>0)
    	{
			...
			$this->excel->getActiveSheet()->setCellValue('A1', 'Lead Owner');
    		...
    		foreach($leads as $lead) {		
    			...
    			$cnt++;
    			if(empty($leads[$cnt]->owner_id) || $leads[$cnt]->owner_id != $lead->owner_id)
    			{
    				$end = $i;
    				$this->excel->getActiveSheet()->setCellValue('A'.$st, $lead->owner_first_name.' '.$lead->owner_last_name);
			...
			$filename='Lead_report_by_owner.xls'   ; //save our workbook as this file name
	...
	public function get_currency_rates() {
		$currency_rates = $this->report_lead_owner_model->get_currency_rate();
    	$rates = array();
