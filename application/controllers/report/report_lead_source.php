<?php
class report_lead_source extends crm_controller {
...
public $userdata;
    ...
    function __construct()
	{
		...
		$this->load->model('report/report_lead_source_model');
    ...
    {  
    	...
    	$this->load->vars($data);
    	$this->load->view('report/report_lead_source');	
    }
    ...
    public function get_lead_report() {
    	...
    	$res = $this->report_lead_source_model->getLeadReportBySourc($options);   	
    	...
    	if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) AND strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {   		
   			$this->load->view('report/lead_report_source_view',$data);
		}else{
    		return $this->load->view('report/lead_report_source_view',$data,true);
		}    	
    }
    ...
    public function excelExport() {
    	...
    	if($res['num']>0)
    	{
			...
			$this->excel->getActiveSheet()->setCellValue('A1', 'Lead Source');
    		...
    		foreach($leads as $lead) {		
    			...
    			$cnt++;
    			if(empty($leads[$cnt]->lead_source) || $leads[$cnt]->lead_source != $lead->lead_source)
    			{
    				$end = $i;
    				$this->excel->getActiveSheet()->setCellValue('A'.$st, $lead->lead_source_name);
			...
			$filename='Lead_report_by_sources.xls'   ; //save our workbook as this file name
	...
	public function get_currency_rates() {
		$currency_rates = $this->report_lead_source_model->get_currency_rate();
    	$rates = array();
