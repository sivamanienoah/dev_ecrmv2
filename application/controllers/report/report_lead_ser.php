<?php
class report_lead_ser extends crm_controller {
...
public $userdata;
    ...
    function __construct()
	{
		...
		$this->load->model('report/report_lead_ser_model');
    ...
    {  
    	...
    	$data['serv_requ'] 	= $this->report_lead_ser_model->get_serv_req();
		$data['report'] 	= $this->get_lead_report();

    	$this->load->vars($data);    	 	
    	$this->load->view('report/report_lead_ser');	
    }
    ...
    public function get_lead_report() {
		...
		$options['ser_requ'] = $this->input->post('ser_requ');
		
    	if($this->userdata['level'] >1){
			$options['cust_id'] =  $this->report_lead_region_model->getCustomerByLocation();						
		}
		
    	$res = $this->report_lead_ser_model->getLeadReportBySerReq($options);   	
    	...
    	if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) AND strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {   		
   			$this->load->view('report/lead_report_ser_view',$data);
		}else{
    		return $this->load->view('report/lead_report_ser_view',$data,true);
		}    	
    }
    ...
    public function excelExport() {
    	...
    	if($res['num']>0)
    	{
			...
			$this->excel->getActiveSheet()->setCellValue('A1', 'Services');
			...
			$filename='Lead_report_by_services.xls'   ; //save our workbook as this file name
	...
	public function get_currency_rates() {
		$currency_rates = $this->report_lead_ser_model->get_currency_rate();
    	$rates = array();
