<?php
class report_lead_region extends crm_controller {
    ...
    public function get_lead_report() {
    	...
    	$res = $this->report_lead_region_model->getLeadReportByRegion($options);
    	...
    	if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) AND strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {    		
   			$this->load->view('report/lead_report_region_view',$data);
		}else{
    		return $this->load->view('report/lead_report_region_view',$data,true);
		}    	
    }
    ...
    public function excelExport()
    {
    	...
    	if($res['num']>0)
    	{
			...
			if(!empty($options['locname']) && $options['locname']!='null'){
				$sort = 'location_name';
			}elseif (!empty($options['statename']) && $options['statename'] != 'null'){
				$sort = 'state_name';
			}
			elseif (!empty($options['countryname']) && $options['countryname']!='null'){
				$sort = 'country_name';
			}else{
				$sort = "region_name";
			}
    		...
    		foreach($leads as $lead) {    			
    			...
    			$cnt++;
    			//if(empty($leads[$cnt]->region_name) || $leads[$cnt]->region_name != $lead->region_name)
    			if(empty($leads[$cnt]->$sort) || $leads[$cnt]->$sort != $lead->$sort)
    			{
    				$end = $i;
    				//$this->excel->getActiveSheet()->setCellValue('A'.$st, $lead->region_name);
    				$this->excel->getActiveSheet()->setCellValue('A'.$st, $lead->$sort);
    				...
    				$this->excel->getActiveSheet()->getStyle('A'.$st.':A'.$end)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::VERTICAL_CENTER);
    				//$this->excel->getActiveSheet()->getStyle('A'.$st.':A'.$end)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			...
			$filename='Lead_report_by_region.xls'   ; //save our workbook as this file name
