<?php
class report_lead_industry extends crm_controller {
...
public $userdata;
    ...
    function __construct()
	{
		...
		$this->load->model('report/report_lead_industry_model');
    ...
    {  
    	...
    	$this->load->vars($data);    	 	
    	$this->load->view('report/report_lead_industry');
    }
    ...
    public function get_lead_report() {
		...
		$options['industry'] 	= $this->input->post('industry');
		
    	if($this->userdata['level'] >1){
			$options['cust_id'] =  $this->report_lead_region_model->getCustomerByLocation();						
		}
		
    	$res = $this->report_lead_industry_model->getLeadReportByIndustry($options);   	
    	...
    	if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) AND strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) === 'xmlhttprequest') {   		
   			$this->load->view('report/lead_report_industry_view',$data);
		}else{
    		return $this->load->view('report/lead_report_industry_view',$data,true);
		}    	
    }
    ...
    public function excelExport() {
    	...
    	if($res['num']>0)
    	{
			...
			//name the worksheet
			$this->excel->getActiveSheet()->setTitle('Leads by Industry');
											
			//set cell A1 content with some text
			$this->excel->getActiveSheet()->setCellValue('A1', 'Industry');
			$this->excel->getActiveSheet()->setCellValue('B1', 'Lead No.');
			$this->excel->getActiveSheet()->setCellValue('C1', 'Lead Title');
			$this->excel->getActiveSheet()->setCellValue('D1', 'Customer');
			$this->excel->getActiveSheet()->setCellValue('E1', 'Region');
    		...
    		foreach($leads as $lead) {		
    			...
    			$cnt++;
    			if(empty($leads[$cnt]->industry) || $leads[$cnt]->industry != $lead->industry)
    			{
    				$end = $i;
    				$this->excel->getActiveSheet()->setCellValue('A'.$st, $lead->industry);
			...
			//Column Alignment
			$this->excel->getActiveSheet()->getStyle('B2:B'.$i)->getNumberFormat()->setFormatCode('00000');
			
			$filename='Lead_report_by_industry.xls'   ; //save our workbook as this file name
    	...
    	redirect('/report/report_lead_industry/');
    }
    
	public function get_currency_rates() {
		$currency_rates = $this->report_lead_industry_model->get_currency_rate();
    	$rates = array();
