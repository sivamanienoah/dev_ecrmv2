...
foreach ($results as $result)
{
	...
	{	
		...
		if($result['userid_fk'] == $userdata['userid']){
			$td = ' ';
			$tdcon = ' ';
		} else {
			$utuser = $ut['user_name'];
		...
		if($result['created_byid'] == $userdata['userid']) {
			$tdowner = ' ';
			$tdownercon = ' ';
		} else {
	...
	<table class="great-task-table" border="0" cellpadding="0" cellspacing="0" id="user-{$uk}">
		<tr class="row-header">
			<td colspan="1" class="user">{$use}  Production</td>
...
EOD;
	...
	<table class="great-task-table" border="0" cellpadding="0" cellspacing="0" id="unapprove-user-{$uk}">
		<tr class="row-header">
			<td colspan="1" class="user">{$ut['user_name']} - Personal</td>
			<td>Remarks</td>
			<td>Assigned By</td>
			<td>Assigned To</td>
			<td align="center">Status</td>
	...
	foreach ($user_tasks as $uk => $ut)
	{
		...
		/*mycahanges*/
			...
			if($uk == $userdata['userid']) {
				$title = "<h3>Tasks Assigned To Me</h3>";
			} else if($uk == in_array($uio,$b)) {
				$title = "<h3>Tasks Assigned By Me</h3>";
			}
		echo <<< EOD
		{$title}
	<table class="great-task-table" border="0" cellpadding="0" cellspacing="0" id="user-{$uk}">
