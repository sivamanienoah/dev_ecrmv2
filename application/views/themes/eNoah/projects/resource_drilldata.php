...
<div>
<?php
$perc_tot_hr = $perc_tot_cost = $perc_tot_directcost = $calc_tot_hour = $calc_tot_cost =  $calc_tot_directcost = 0;
if(!empty($tbl_data)) {
	...
	foreach($tbl_data as $dept=>$us_ar) {
		if($filter_sort_by=='asc') {
			...
			} else if($filter_sort_val=='direcctcost') {
		...
		foreach($us_sort_ar as $p_name=>$proj_ar) {
			...
			foreach($usr_arr as $pkey=>$pval) {
				...
				$per_hr = $per_cost = $per_hr = $per_cost = 0;
