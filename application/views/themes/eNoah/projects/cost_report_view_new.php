    ...
    <div class="inner">
        <?php if($this->session->userdata('viewPjt')==1) { ?>
			...
			<div id="filter_section">
				...
				<form action="<?php echo site_url('projects/dashboard/cost_report_new')?>" name="project_dashboard" id="project_dashboard" method="post">					
					<input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>" />
						<div style="border: 1px solid #DCDCDC;">
							<table cellpadding="0" cellspacing="0" class="data-table leadAdvancedfiltertbl" >
								<tr>
									...
									<td class="tblheadbg" width="15%">PROJECT</td>
								...
								<tr>	
									...
									<td class="proj-dash-select">
										<select multiple="multiple" title="Select Project" id="project_ids" name="project_ids[]">
											<?php
											//$project_ids=array();	
											if(count($project_ids)>0 && !empty($project_ids)) { ?>
													<?php foreach($project_ids as $proj) {?>
														<option <?php echo in_array($proj->project_code, $sel_project_ids)?'selected="selected"':'';?> value="<?php echo $proj->project_code;?>" title="<?php echo $proj->project_name;?>"><?php echo $proj->project_name;?></option>
											<?php } } ?>
										</select>
									</td>
									<td class="proj-dash-select">
