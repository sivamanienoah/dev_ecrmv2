...
<?php
	...
	if (is_array($projects_data) && count($projects_data) > 0) {
		...
		$total_mile_uc_amt = 0;
		$total_mile_pl_amt = 0;	
		$full_total_mile_amount_inv_raised = 0;
		foreach($projects_data as $record){
			...
			$contributePercent 			= (isset($total_amount_inv_raised) && !empty($total_amount_inv_raised)) ? round((($total_amount_inv_raised - $total_cost)/$total_amount_inv_raised)*100) : '';
			// $contributePercent = round($total_dc_hours, 2);
			$profitloss        = (isset($total_amount_inv_raised) && !empty($total_amount_inv_raised)) ? round($total_amount_inv_raised - $total_cost) : '';
			$profitlossPercent = (isset($total_amount_inv_raised) && !empty($total_amount_inv_raised)) ? round(($profitloss/$total_amount_inv_raised)*100) : '';
