		...
		<div style="margin-top:20px;" class="tasks-mgmt">
				<form id="set-job-task" onsubmit="return false;">
					...
					<!--<div class="buttons task-init toggler">
						<button type="button" class="positive" onclick="$('.toggler').slideToggle();">Add General Task</button>
					</div>-->
				</form>
		</div>
				...
				<input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>" />
					<table border="0" cellpadding="0" cellspacing="4" class="task-add">
						<tr>
							...
							<td>
								<select id="task_search_user" name="task_search_user" class="textfield width100px">
		...
		<!-- edit task -->
			...
			<?php
				...
				<tr>
					...
					<?php if(in_array($uio,$b)) {?>
					<td>
						<td><input type="text" name="task_actualstart_date" class="edit-actualstart-date textfield pick-date width100px" readonly/></td>
					</td>
					<?php } else { ?>
