...
function chk_cat_name() {
	...
	var type		        = 'job_categories';
	var params 				= {name: category_name, id: category_hidden, type: type};
	params[csrf_token_name] = csrf_hash_token;
	
	if (category_name == "") {
		$('#cat_name_msg').show();
		$('#cat_name_msg').append("<span class='ajx_failure_msg'>Category Name Required.</span>");
		return false;
	} else {
		$.ajax({
			...
			success: function(data) {
				if(data == 'fail') {
					$('#cat_name_msg').show();
					$('#cat_name_msg').append("<span class='ajx_failure_msg'>Category Name Already Exists.</span>");
					return false;
				} else {
